...
module.exports.createUser = (fullname, email, password, callback) => {
    ...
    if (fullname == "" || fullname == null || email == "" || email == null || password == "" || password == null) {
        logger.info('Registration failed as credentials are not completely filled.')
    } else {
            ...
            if (err) {
                ...
                connection.query(`INSERT INTO user ( fullname, email, user_password, 
                    ...
                    if (err) {
                        ...
                        callback(rows, null);
                        logger.info('User ' + fullname + ' has successfully registered with email ' + email);
                    }
                    connection.release();
                });
            }
            ...
            if (err) {
                ...
                connection.query(`UPDATE user SET role_id =${newRoleId} WHERE user_id=${recordId}`, (err, rows) => {
                    if (err) {
                        ...
                        logger.info('User ' + recordId + ' has been assigned a new role of id ' + newRoleId);
                        resolve(rows);
                    }
                    connection.release();
                });
...
module.exports.getUserData = (pageNumber, search) => {
    ...
    //     }
    userDataQuery = `CALL sp_get_paged_user_records(?,?,?, @total_records); SELECT @total_records total_records;`;
    return new Promise((resolve, reject) => {
        //I referred to https://www.codota.com/code/javascript/functions/mysql/Pool/getConnection
        //to prepare the following code pattern which does not use callback technique (uses Promise technique)
        pool.getConnection((err, connection) => {
            if (err) {
                ...
                connection.query(userDataQuery, [limit, offset, search], (err, results) => {
                    if (err) {
                        logger.error('Error: '+err)
                        reject(err);
                    } else {
            ...
            if (err) {
                ...
                connection.query(userDataQuery, [recordId], (err, results) => {
                    if (err) {
                        ...
                        logger.info('User ' + recordId + ' data is being fetched')
                        resolve(results);
                    }
                    connection.release();
                });
            ...
            if (err) {
                ...
                connection.query(userDataQuery, [search], (err, results) => {
                    if (err) {
                        ...
                        logger.info(search + ' user record is fetched');
                        resolve(results);
                    }
                    connection.release();
                });
...
//                         reject(err);
//                     } else {
//                         module.exports.created_by_id = results[0].created_by_id;
//                         logger.info('Design File id ' + recordId + ' record is fetched');
//                         resolve(results);
//                     }
//                     connection.release();
//                 });
            ...
            if (err) {
                ...
                connection.query(`UPDATE file SET design_title =? , design_description=? WHERE file_id=?`, [title, description, recordId], (err, rows) => {
                    if (err) {
                        ...
                        logger.info('Design File id ' + recordId + ' has been successfully updated');
                        resolve(rows);
                    }
                    connection.release();
                });
...
module.exports.createOneEmailInvitation = (userData, recipientName, recipientEmail) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            logger.info(userData.fullname + '(' + userData.email + ')' + ' has successfully sent an email to ' + recipientName + '(' + recipientEmail + ')');
            resolve({ status: 'success', description: 'Email sent' });
        } catch (error) {
            logger.error('Mail has not been submitted: ' + error);
