...
module.exports.uploadFile = async(file) => {
    ...
    cloudinary.uploader.upload(file.path, { upload_preset: 'upload_to_design' })
        .then((result) => {
            ...
            logger.info('File successfully uploaded. imageURL: ' + result.url + ' publicId: ' + result.public_id);
            return resolve (data);

        }).catch((error) => {

            let message = 'fail';
            logger.error('File uploading ' + message + 'as ' + error);
            return reject(error);
        });
    });
            ...
            if (err) {
                ...
                connection.query(query, [publicId, imageURL, designTitle, designDescription, userId], (err, rows) => {
                    if (err) {
                        ...
                        logger.info('File id ' + publicId + ' created by ' + userId + ' has been successfully inserted.')
    ...
    return new Promise((resolve, reject) => {
        ...
        pool.getConnection((err, connection) => {
            if (err) {
                ...
                connection.query(designFileDataQuery, [limit, offset, userId, search], (err, results) => {
                    if (err) {
...
module.exports.getFileDataByUserId = (userId, pageNumber) => {
    ...
    designFileDataQuery = `CALL sp_get_file_records(?,?,?, @total_records); SELECT @total_records total_records;`;
    return new Promise((resolve, reject) => {
        //I referred to https://www.codota.com/code/javascript/functions/mysql/Pool/getConnection
        //to prepare the following code pattern which does not use callback technique (uses Promise technique)
        pool.getConnection((err, connection) => {
            if (err) {
                ...
                console.log('Executing query to obtain 1 page of 3 data');
                connection.query(designFileDataQuery, [limit, offset, userId], (err, results) => {
                    if (err) {
                        logger.error('Error on query on reading data from the file table'+ err);
                        reject(err);
                    } else {
