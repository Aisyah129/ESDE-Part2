...
exports.processDesignSubmission = async(req, res, next) => {
    ...
    await fileDataManager.uploadFile(file)
        ...
        .catch((error) => {
            ...
            logger.error(message+' of user id '+userId)
            res.status(500).json({
                message: message
            });
        })
...
//         if (userManager.created_by_id != req.body.userId) {
//             logger.warn("Failed: User "+ req.body.userId+" trying to access design by user "+ userManager.created_by_id);
//             res.status(403).send("Please only update and access your own design! Don't Steal!!")
//             return;
//         }
...
exports.processUpdateOneDesign = async(req, res, next) => {
    ...
    // if (userManager.created_by_id != req.body.userId) {
    //     logger.warn("Failed: User "+ req.body.userId+" trying to update design by user "+ userManager.created_by_id);
    //     res.status(403).send("Please only update and access your own design! Don't Steal!!")
    //     return;
    // }
